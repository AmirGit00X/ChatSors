...
function get_on($id){
	$list = json_decode(file_get_contents("data/online.json"), TRUE);
	$h = false;
	$t = 0;
	//SendMessage("235035502",json_encode($list)."/".$id."\n".explode("/",$list[0])[1].explode("/",$list[0])[0]);
	for($c = 0; $c <= count($list)-1; $c++){
	    if(explode("/",$list[$c])[0] == $id){
	        $h = true;
	        $list2 = $list;
	        $list2[$c] = $id."/".time();
	        file_put_contents("data/online.json",json_encode($list2));
	    }
		if(explode("/",$list[$c])[0] == $id && explode("/",$list[$c])[1] > time()-60){
			return "60";
		}
		if(explode("/",$list[$c])[1] >= time()-60){
		    $list[$c] = explode("/",$list[$c])[0]."/0";
		    $list[$id] = $id."/0";
	        file_put_contents("data/online.json",json_encode($list));
			return explode("/",$list[$c])[0];
		}
	}
	if(!$h) {
	$list[count($list)] = $id."/".time();
	file_put_contents("data/online.json",json_encode($list));
	}
	return "";
}
...
if(!file_exists("data/".$from_id.$fromid.".txt") && (!empty($from_id) || !empty($fromid))){
	@$fp = fopen("data/".$from_id.$fromid.".txt","w");
	fwrite($fp,"false");
	fclose($fp);
}
...
$vip = file_get_contents("data/$from_id.txt");
$numchat = $user["userfild"]["$from_id"]["numchat"];
if ($vip == "false") {
if ($numchat > -4 || empty($numchat)){
//$listchat = json_decode(file_get_contents("data/online.json"));
//SendMessage("ID",$listchat."-");
//$randomchat = $listchat[rand(0,count($listchat)-1)];
$randomchat = get_on($from_id);
if($randomchat == "60"){
	SendMessage($from_id, "🔶 برای ارسال درخواست جدید باید حداقل ۶۰ ثانیه از درخواست قبلی شما گذشته باشد...");
	die();
} 
if ($randomchat != ""){
...
{
	SendMessage($from_id,"🔴 کاربری برای گفتگو یافت نشد... اگر تا ۶۰ ثانیه بعد گفتگو آغاز نشد درخواستتان را تکرار کنید...");
	die();
}
}
else
{
	SendMessage($from_id,"▫️شما فقط میتوانید ۱۰ بار بصورت رایگان چت کنید، برای رفع محدودیت حساب خود را ویژه کنید...");
	die();
}
}
else
...
$like2 = $user["userfild"]["$from_id"]["like"];
$deslike2 = $user["userfild"]["$from_id"]["deslike"];
//SendMessage("ID",json_encode($listchat)."-".json_encode($user));
			jijibot('sendmessage',[
	'chat_id'=>$chat_id,
	'text'=>"🤖 پیام سیستم :
...
elseif($data == "by"){
	jijibot('sendmessage',[
	'chat_id'=>$chatid,
	'text'=>"📍 شما با پرداخت 3000 هزار تومن میتوانید حساب خودتون رو ویژه کنید و از امکانات فوق العاده ربات استفاده کنید
...
'reply_markup'=>json_encode([
    'inline_keyboard'=>[
	[
	['text'=>"🚀 خرید",'url'=>"$web/chat/other/pay.php?amount=3000&callback=$web/chat/other/by-2000.php?id=$fromid"]
	],
              ]
        ])
    		]);
...
'reply_markup'=>json_encode([
    'inline_keyboard'=>[
	[
	['text'=>"🚀 خرید",'url'=>"$web/chat/other/pay.php?amount=3000&callback=$web/chat/other/by-2000.php?id=$from_id"]
	],
              ]
        ])
    		]);
}
elseif(file_get_contents("data/$from_id.txt") != "true"){
...
file_put_contents("data/$from_id.txt","true");
$user = json_encode($user,true);
file_put_contents("data/user.json",$user);
}
}
...
file_put_contents("data/$forward_from_id.txt","true");
...
file_put_contents("data/$textmassage.txt","true");
...
file_put_contents("data/$forward_from_id.txt","false");
...
file_put_contents("data/$textmassage.txt","false");
